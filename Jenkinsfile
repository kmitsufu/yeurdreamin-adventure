...
node {
    ...
    withCredentials([file(credentialsId: JWT_KEY_CRED_ID, variable: 'jwt_key_file')]) {
        stage('Authorize ORG') {
            if (isUnix()) {
                rc = sh returnStatus: true, script: "${toolbelt}/sfdx auth:jwt:grant --client-id ${CONNECTED_APP_CONSUMER_KEY} -o ${HUB_ORG} --jwt-key-file ${jwt_key_file} --instance-url ${SFDC_HOST}"
            } else {
